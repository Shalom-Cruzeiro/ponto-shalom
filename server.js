...
app.post('/api/funcionarios',auth,async(req,res)=>{try{const{nome,cargo,hora_inicio,hora_fim,dias_semana}=req.body;if(!nome)return res.status(400).json({erro:'Nome obrigatorio'});const r=await dbRun("INSERT INTO funcionarios(loja_id,nome,cargo,hora_inicio,hora_fim,dias_semana)VALUES(?,?,?,?,?,?)",[req.session.lojaId,nome,cargo||'Funcionario',hora_inicio||null,hora_fim||null,parseInt(dias_semana)||5]);res.json({id:r.lastID,nome,cargo})}catch(e){res.status(500).json({erro:e.message})}})
app.put('/api/funcionarios/:id',auth,async(req,res)=>{try{const{nome,cargo,hora_inicio,hora_fim,dias_semana}=req.body;await dbRun("UPDATE funcionarios SET nome=?,cargo=?,hora_inicio=?,hora_fim=?,dias_semana=? WHERE id=? AND loja_id=?",[nome,cargo,hora_inicio||null,hora_fim||null,parseInt(dias_semana)||5,req.params.id,req.session.lojaId]);res.json({ok:true})}catch(e){res.status(500).json({erro:e.message})}})
