...
function renderFuncBody() {
  ...
  fb.innerHTML = FUNCS.map(f => `<tr>
    ...
    <td><div style="display:flex;gap:6px;"><button class="btn btn-sm btn-i" onclick='abrirModal(${JSON.stringify(f).replace(/'/g,"&#39;")})'>
      <i class="ti ti-pencil"></i> Editar
    </button><button class="btn btn-sm" onclick='abrirFolgas(${f.id},"${f.nome.replace(/"/g,"")}")'>
      <i class="ti ti-calendar"></i> Folgas
    </button></div></td>
  </tr>`).join('')
}
...
async function salvarFolgas() {
  var funcId = document.getElementById('folgas-func-id').value
  var mes = parseInt(document.getElementById('folgas-mes').value)
  var ano = parseInt(document.getElementById('folgas-ano').value)
  var diasStr = document.getElementById('folgas-dias').value
  var dias = diasStr ? diasStr.split(',').map(function(d){return parseInt(d.trim())}).filter(function(d){return !isNaN(d)&&d>0&&d<=31}) : []
  try {
    await api('POST', '/folgas', { funcionarioId: funcId, mes: mes, ano: ano, dias: dias })
    document.getElementById('folgas-ok').style.display = 'flex'
    setTimeout(function(){ document.getElementById('folgas-ok').style.display='none' }, 2000)
  } catch(e) { alert('Erro: '+e.message) }
}

function fecharModalFolgas() {
  document.getElementById('modal-folgas').style.display = 'none'
}

async function carregarFolgas(funcId) {
  try {
    var mes = document.getElementById('folgas-mes').value
    var ano = document.getElementById('folgas-ano').value
    var folgas = await api('GET', '/folgas?funcId='+funcId+'&mes='+mes+'&ano='+ano)
    document.getElementById('folgas-dias').value = folgas.map(function(f){return f.dia}).sort(function(a,b){return a-b}).join(', ')
  } catch(e) { console.error(e) }
}
